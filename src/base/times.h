...
struct Duration {
	...
	constexpr explicit Duration(uint32_t v = std::numeric_limits<uint32_t>::max()) noexcept : value_(v) {
	}
...
struct Time {
	...
	constexpr explicit Time(uint32_t v = std::numeric_limits<uint32_t>::max()) noexcept : value_(v) {
	}
