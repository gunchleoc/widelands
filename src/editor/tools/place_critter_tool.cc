...
int32_t EditorPlaceCritterTool::handle_click_impl(const Widelands::NodeAndTriangle<>& center,
	...
	if (get_nr_enabled() && args->old_bob_type.empty()) {
		...
		do {
			Widelands::Bob* const mbob = mr.location().field->get_bobs().front();
		...
		do {
			...
			if (mr.location().field->nodecaps() & descr.movecaps()) {
				if (!mr.location().field->get_bobs().empty()) {
					//  There is already a bob. Remove it.
					mr.location().field->get_bobs().back()->remove(egbase);
				}
   ...
   Widelands::Map* map) {
	...
	if (!args->new_bob_type.empty()) {
		...
		do {
			...
			} else if (!mr.location().field->get_bobs().empty()) {
				mr.location().field->get_bobs().back()->remove(egbase);
			}
			++i;
		} while (mr.advance(*map));
