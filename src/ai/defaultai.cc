...
void DefaultAI::late_initialization() {
	...
	for (DescriptionIndex building_index = 0; building_index < nr_buildings; ++building_index) {
		...
		if (bld.type() == MapObjectType::PRODUCTIONSITE) {
			...
			// NOCOM Note: this adds woodcutters/lumberjacks, Barbarian Hunter, Atlantean Fisher to the
...
void DefaultAI::update_buildable_field(BuildableField& field) {
	...
	field.supporters_nearby_new.clear();
...
void DefaultAI::consider_productionsite_influence(BuildableField& field,
	...
	if (bo.type == BuildingObserver::Type::kProductionsite) {
		const ProductionSiteDescr* productionsite = dynamic_cast<const ProductionSiteDescr*>(bo.desc);
		for (const auto& supported : productionsite->supported_productionsites()) {
			if (field.supporters_nearby_new.count(supported) != 1) {
				field.supporters_nearby_new[supported] = 0;
			}
			++field.supporters_nearby_new[supported];
		}
	}

	// NOCOM production_hints
