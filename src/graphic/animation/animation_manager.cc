...
uint32_t AnimationManager::load(const LuaTable& table,
	...
	case Animation::Type::kFiles:
		animations_.push_back(
		   std::unique_ptr<Animation>(new NonPackedAnimation(table, basename, filesystem)));
		break;
	case Animation::Type::kSpritesheet:
		animations_.push_back(std::unique_ptr<Animation>(
		   new SpriteSheetAnimation(table, basename, filesystem)));
	}
...
uint32_t AnimationManager::load(const std::string& map_object_name,
                                ...
                                Animation::AnimationFilesystem filesystem,
                                Animation::Type type) {
	const size_t result = load(table, basename, filesystem, type);
